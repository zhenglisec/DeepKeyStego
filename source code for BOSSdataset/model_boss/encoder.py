...
from model_boss.conv_bn_lrelu import ConvBNLRelu
class Encoder(nn.Module):
    def __init__(self, sec_len=8192):
        ...
        layers = [ConvBNLRelu(7, self.conv_channels)]
        for _ in range(self.num_blocks-1):
            layer = ConvBNLRelu(self.conv_channels, self.conv_channels)
            layers.append(layer)

        self.conv_layers = nn.Sequential(*layers)
        self.final_layer = nn.Sequential( nn.Conv2d(self.conv_channels, 1, kernel_size=1),
                                        nn.Sigmoid(),)
...
class Process(nn.Module):
    def __init__(self, sec_len = 1024):
        ...
        self.pre_32x32 = nn.Sequential(
            ...
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.ConvTranspose2d(3, 3, kernel_size=(4, 4), stride=(2, 2), padding=(1, 1), bias=True),
            nn.BatchNorm2d(3, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.ConvTranspose2d(3, 3, kernel_size=(4, 4), stride=(2, 2), padding=(1, 1), bias=True),
            nn.BatchNorm2d(3, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),
        ...
        self.pre_512x256 = nn.Sequential(
    ...
    def forward(self, input):
        if self.sec_len == 32*32:
            ...
            return out
        elif self.sec_len == 512*256:
            input = input.view(-1, 1, 512, 256)
            out = self.pre_512x256(input)
            return out
